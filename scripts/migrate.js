...
async function executarPasso(conn, passo) {
    try {
...
async function migrar() {
    ...
    try {
        for (const passo of colunas) {
            await executarPasso(conn, passo);
        }

        for (const passo of tabelas) {
            try {
                await conn.query(passo.sql);
                console.log(`Tabela ok: ${passo.nome}`);
            } catch (err) {
                console.error(`Erro em ${passo.nome}:`, err.message);
                throw err;
            }
        }
        ...
        if (semSlug.length) {
            console.log(`Slugs gerados para ${semSlug.length} evento(s)`);
        }
    } finally {
        await conn.end();
    }
